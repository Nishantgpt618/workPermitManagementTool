...
import { fetchPermit, approvePermitBySafetyManager, mind } from '../redux/actions';
import { connect } from 'react-redux';
import Card from 'react-bootstrap/esm/Card';

import {Link} from 'react-router-dom';

class ManageSafetyManagerRequests extends React.Component {
    ...
    async mergeFunctions(id) {
        ...
        await this.props.approvePermitBySafetyManager(id)
    ...
    checkManager() {
        if (this.state.currentUser) {
            if (this.state.currentUser.designation === "SafetyManager") {
                return this.props.permits.filter(data => { return data.verifiedSafetyManager === false || data.isActive === false }).map((permit, i) => <div className="permit-item" key={i}><Card>
                    <Card.Body>
                    ...
                    <div className="submitButton" style={{ marginBottom: "20px" }}>
                        {permit.verifiedSafetyManager ? <button type="submit" onClick={() => this.preventDefaultFunction()}>Approved</button> : <button type="submit" onClick={() => this.mergeFunctions(permit._id)}>Approve</button>}
                    </div>
                </Card></div>)
            }
        }
...
export default connect(mapStateToProps, { fetchPermit, approvePermitBySafetyManager, mind })(ManageSafetyManagerRequests);